...
class ModelConverter:
    ...
    def to_tensorrt(self):
        ...
        if self.use_verify:
            ...
            trt_output = self.infer(engine, dummy_input.numpy())
            
            # 출력 비교
            np.testing.assert_allclose(onnx_output, trt_output, rtol=self.rtol, atol=self.atol)
            print("ONNX and TensorRT outputs are within tolerance.")


    # TensorRT 엔진에서 더미 입력으로 추론
    ...
    def infer(engine, input_data):
        ...
        return outputs[0].reshape(engine.get_binding_shape(1))
